...
describe('optimizeSvgString', () => {
  it('antd logo 解析', async () => {
    expect(await optimizeRawSVG(antdRawSvg)).toBe(antdOptSvg.trim());
  });
  it('background 解析', async () => {
    expect(await optimizeRawSVG(bgRawSvg)).toBe(bgOptSvg.trim());
  });
});

describe('urlToRawString', () => {
  it('bgRaw 正常解析', async () => {
...
describe('nodeToOptSVG', () => {
  beforeAll(() => {
    const innerHTML = `
<svg id="antd" width="200px" height="200px" viewBox="0 0 200 200" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
